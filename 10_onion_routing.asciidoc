...
Obtain the session key for this hop derived from the blinding factor generated by the prior hop.
...
First, Bob *extends* the onion payload by 1300 bytes and fills them with +0+ values. Now the onion packet is 2600 bytes long, with the first half containing the data Alice sent and the next half containing zeroes. This operation is shown in <<bob_extends>>:
