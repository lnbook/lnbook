...
[Note]
====
There is a lot of mathmatical theory and modelling behind the uncertainty of the liquidity in the channels.
We will just mention them briefly here for completness and refere you to research work conducted by one of the authors of the book if you want take a deep dive.
So with the tools from probability theory one can properly express the uncertainty about the liquidity in a channel.
Assume we have a probability function `P` that encodes the uncertainty by assigning a likelihood to each liquidity value `x` via `P(X =x)` we can express the Entropy (or uncertainty) we have about a channel as: H(P) = \sum_{x = 0}^c P(X = x) · log(P(X = x)) which is measured in bits.
When making a successful or unsuccessfull attempt we update the probability for the event `A` accordingly by going to the conditional probability `P(X|A)`.
This updated probability can be used to compute the information gain as the Kullback Leibler divergence between the prior probability `P(X)` and the posteriori probability `P(X|A)` which in many cases is will just be the difference of entropy values between the two distributions.
As mentioned before fundamental work work about modeling the uncertainty intervals of the channel liquidity can be found in the Paper https://arxiv.org/abs/2103.08576[Security and Privacy of Lightning Network Payments with Uncertain Channel Balances] by Pickhardt et. al.
====
