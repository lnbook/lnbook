...
_encrypted_ message transport layer. As we learned in chapter XXX, the Lightning
...
this requirement allows for de-coupled de-synchronized evolution of the Lightning
...
===== Lightning's Protobuf Inspired Message Extension Format: `TLV`
...
acceptable within the context of Lightning, was many messages contain a
...
chapter once we dsecribe how the Lightning Protocol is upgraded in practice and
in theory.

=== Feature Bits & Protocol Extensibility

As the Lightning Network is a decentralized system, no one entity can enforce a
...
upgrade mechanisms embedded in the Lightning Network, at most, only the
participants that wish to use these new Lightning Network feature need to
...
gradient of possible upgrade mechanisms within the Lightning Network. In this
