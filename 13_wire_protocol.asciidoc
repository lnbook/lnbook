...
the protocol. By the end of this chapter, an aspiring reader should be able to
...
will gain a deep understanding with respect to the various upgrade mechanisms
...
compilers" exist in dozens of languages which act as a bridge that allows any
language to encode a Protobuf that will be able to be decoded by a compliant decoder
...
likely familiar with the concept of backwards compatibility. In simple terms,
...
Protobuf extensibility examples above, backwards compatibility is achieved by
ensuring that new additions to the proto format don't break the known portions
of older readers. Forwards compatibility on the other hand is just as important
...
curious reader might ask "why not just use Protobufs"?
The Lighting developers would respond that we're able to have the best of the
...
recipient for settlement. The roll out of MPP was coupled with a new
