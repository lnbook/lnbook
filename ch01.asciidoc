...
The Revocable Sequence Maturity Contracts (RSMC), which allowed participants in a multi-signature payment "channel" to invalidate outdated transactions in the first version of the Lightning Network, resolved the weakness in the 'nSequence' field. With RSMC, a penalty mechanism was introduced where the entire transaction output can be claimed by a participant if the other participant published an outdated transaction.
