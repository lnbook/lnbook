...
When Gloria receives the Onion packet he will extract the public key from the unencrypted part of the onion package.
Gloria then uses ECDH to derive the shared secret using that ephemeral public key which he'll use to process the packet in full.
