...
Alice can make this attractive to both Bob and Wei, by offering them fees of 1 gold coin each, if they transmit her payment to Gloria.
...
If she does not provide the secret by this time, Wei's deposit will be refunded by the escrow service and the contract becomes invalid._
----
...
The combination of the condition, and a timeout on the condition gives the payment a trait we commonly refer to as atomicity: either the payment happens, or the receiver is refunded the funds.
...
Finally, we require that none of the intermediaries are able to "contaminate" the payment beyond giving incorrect directions to the party that directly follows them.
...
Instead, answer to the second question has a very large solution space, meaning there are several algorithms and heuristics used in the network today.
...
However, the lack of encryption, end-to-end integrity checks, and arbitrary choice of routes make this a poor system to use in a _payment network_.
...
Although, in an ideal model only a precise amount of information is leaked by a route, in practice several "side channels' exist, that may allow an adversary to deduce more information about a route.
...
On the other hand there are hardly any plausible reasons other than its wish to act maliciously why a TOR node might not be able to forward an onion.
...
This is nice as it allow a payer to initiate a payment without the necessity to ask the payee for an invoice and payment hash first.
...
We can see that Alice has created some per hop data for Gloria.
The short channel id is set to 0 signaling Gloria that this payment is intended to be for her.
Note that this example is slightly simplified, in that Gloria can also use attributes of the onion packet format itself to be able to know when she's the final hop.
The amount to forward is set to 3000.
On the incoming HTLCs Gloria should have seen that exact amount.
...
Finally the CLTV delta which Gloria should use to forward the payment is also set to block height 800 (the current height minus Gloria's CLTV grace delta) as Gloria is the final hop.
...
These arbitrary key-value pairs can be used to extend the protocol in an end-to-end manner, as in many cases, only the sender and receiver need to know how to interpret the data.
In the next diagram we can see how the per hop payload for Gloria looks like.
...
Since Gloria is the final recipient of the payment, we only have 65 bytes worth of data to fill with actual content.
The remaining bytes are filled with random bytes to pad out the packet in an unpredictable manner.

Taking a step back, before Alice is able to prepare the remainder of the packet, she needs to generate an ephemeral key (a key only used once).
...
When Gloria receives the Onion packet she will extract the public key from the unencrypted part of the onion package.
Gloria then uses ECDH to derive the shared secret using that ephemeral public key which she'll use to process the packet in full.
The properties of ECDH make it such that only Alice and Gloria are able to derive the corresponding shared secret.

After the encrypted Onion for Gloria is created Alice will create the next outer layer by creating the onion for Wei.
...
The per hop data follows the same structure as the per hop data for Gloria.
...
She sets the amount to 3000 satoshi as this is the amount that Gloria is supposed to receive.
...
Luckily the ephemeral keys that Alice used for the ECDH with Gloria can be derived from the ephemeral key that she used for Wei.
Thus after Wei decrypts his layer he can use the shared secret and his ephemeral public key to derive the ephemeral public key that Gloria is supposed to use and store it in the header of the Onion that she forwards to Gloria.
The exact progress to generate the ephemeral keys for every hope will be explained at the very end of the chapter.
Similarly it is important to recognize that Alice removed data from the end of Gloria's onion payload to create space for the per hop data in Wei's onion.
...
In that case outstanding HTLCs produce additional onchain footprint and costs.
...
Thus the HTLCs which are below the dust limit can be understood as additional fees in the commitment transaction.
