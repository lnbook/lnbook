...
In this section we will finally unpack how payment channels can be connected to a network of other users' payment channels.
This allows our gamer Gloria to receive funds from her fans without being required to maintain a separate channel with every one of her fans who want to tip her.
...
Another important detail is that, due to the use of Onion routing, intermediary nodes will not know who is the initiator of the payment and who is the final recipient.
This enables fans to use intermediary nodes to pay Gloria, without leaking private information and without risking theft.
This ability to connect payment channels in a route is the main value proposition behind the lightning network.

This also provides scaling benefits to the Bitcoin network.
A single open channel between two users already allows those users to transact off-chain an arbitrary number of times without making an on-chain payment.
But a network of channels allows any two arbitrary users to transact on the Lightning Network, without the need to open further channels, as long as there is a well-funded path between them.

In this chapter we will first discuss in a non technical way how, for the purposes of Lightning, the Bitcoin Network is not a _transaction settlement_ layer but a _contract settlement_ layer.
Later in the chapter, technical concepts such as Hashed Time Locked Contracts (HTLCs) are explained.
We will discuss how they are included in the commitment transactions and how neighboring nodes communicate with each other to set the HTLCs up and settle them.
Finally, we will discuss the SPHINX mix Format that enables onion routing, which is what grants payments a high degree of privacy.
