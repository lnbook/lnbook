...
This hash is typically referred to as the "payment hash", with the secret pre-image being called the "payment pre-image".
...
Alice can create a transaction output that pays, say 50,000 satoshi with a locking script above, replacing <H> with the hash value provided by Dina. Then, Alice can sign this transaction and offer it to Bob. Bob can spend this output with an unlocking script containing the secrect pre-image value +R+.

The unlocking script and locking script would combined to produce:

----
<R> OP_HASH160 <H> OP_EQUALVERIFY
----

The Bitcoin Script engine would evaluate this script as follows:

1. latexmath:[R] is pushed to the stack
1. The OP_HASH160 operator takes the value latexmath:[R] on the stack and hashes it, pushing the result latexmath:[H_R_] to the stack
...
[TIP]
====
The OP_HASH160 operator calcualtes a _double hash_ using two different algorithms. First, it uses the SHA256 hash algorithm, then it hashes the result again with the RIPEMD160 hash allgorithm. This is done to reduce the size of the script and save transaction fees by only storing 160 bits instead of 256 bits. The use of HASH160 is a common space-saving practice in Bitcoin Script.
====
