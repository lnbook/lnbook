...
[[alice_bob_contract]]
====
_Alice will reimburse Bob with 12 gold coins if you can show a valid message that hashes to:_ +0575...f6b3+
====
...
Alice has received an invoice from Dina requesting a payment of 50,000 satoshi and containing the payment hash Dina produced from her secret.

Alice now needs to pay Dina, by sending a payment via Bob and Chan. As with the gold coin example, Bob and Chan will get paid a small routing fee for their assistance in getting this payment delivered.

Alice can create a transaction output that pays, say 50,200 satoshi with a locking script above, replacing <H> with the hash value +0575...f6b3+ provided by Dina. Then, Alice can sign this transaction and offer it to Bob:
...
1. The OP_EQUAL operator compares latexmath:[H_R_] and latexmath:[H]. If they are equal, the result is +TRUE+, the script is complete and the payment is verified.

It's important to note something about how this script executes, because it plays a role in the fairness protocol and ensures no one can cheat.

To spend this HTLC on-chain, Bob would have to know the secret +R+, as it is required for the HTLC to be validated by the Bitcoin blockchain. Furthermore, to redeem the HTLC, Bob effectively *broadcasts the secret* because he has to put it in the spending transaction and that transaction is broadcast and recorded on the Bitcoin blockchain. Bob can't spend the HTLC without revealing +R+ to the whole world!
