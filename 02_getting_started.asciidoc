...
[TIP]
====
Note that Lightning explorers just illustrate information that can already be queried from the gossip protocol (that's also how they obtain those data). So it is recommended to use commanline tools or a privacy-preserving browser (e.g., Tor browser), if a user care about their privacy. An alternative solution is that, users can build an open-source explorer (for example, using https://github.com/chemicstry/recksplorer) themselves and query data from it.
====
