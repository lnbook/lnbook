...
Users have the highest degree of control by running their own Bitcoin node and Lightning node. However, Lightning nodes can also use a lightweight Bitcoin client (commonly referred to as Simplified Payment Verification (SPV)) to partially validate the correctness of their blockchain. Analogously, within the context of the Lightning Network, there exists a gradient between a "full" and a lightweight Lightning client.
...
* A Bitcoin node that stores blockchain data, is able to broadcast transctions, and communicate with other Bitcoin nodes.
...
* A payment forwarding systems that is able to establish and tear down payment circuits.
* A contract monitoring violation system that is able to monitor and punish renegading peers
* A channel state machine that is able to manage the payment (propose, sending, receive, etc) life-cycle for a particular channel.
...
Lightning wallets can be installed on a variety of devices, including laptops, servers, and mobile devices. To run an effective full Lightning routing node (one that participates in "gossip" and creates its own map of the network for path finding and routing) you will need to use a server or desktop computer, as mobile devices and laptops are usually not powerful enough in terms of capacity, processing, battery life, and connectivity.
...
|===
| Application   | Device  | LN Node     | Bitcoin Node               | Keystore     | Path Finding | Channel Creation
| lnd           | Server  | Full Node   | Bitcoin Core/btcd/neutrino | User Control | User Control | User Control
| c-lightning   | Server  | Full Node   | Bitcoin Core               | User Control | User Control | User control
| Eclair Server | Server  | Full Node   | Bitcoin Core/Electrum      | User Control | User Control | User Control
| Zap Desktop   | Desktop | Full Node   | Bitcoin Core/btcd/neutrino | User Control | User Control | User Control
| Eclair Mobile | Mobile  | Lightweight | Electrum                   | User Control | User Control | User Control w/ Default Channel to Wallet Creator
| Breez Wallet | Mobile   | Full Node   | Bitcoin Core/btcd/neutrino | User Control | User Control | User Control w/ Default Channel to Wallet Creator
| Phoenix Wallet | Mobile | Lightweight  | Electrum  | User Control | Third-Party | Sole Channel to Wallet Creator
| Blue Wallet | Mobile | None | None | Custodial | Third-Party | Managed by Third-Party
|===
