...
* https://1ml.com/[1ML] (mainnet + testnet)
* https://lightblock.me/[LightBlock] (mainnet)
* https://explorer.acinq.co/[ACINQ's Lightning Explorer], with fancy visualization
* https://www.robtex.com/lightning/node/[Robtex Bitcoin Lightning Explorer]

However, using Lightning explorer may introduce privacy issue, because, if users are careless, the website may track their IP addresses and collect their behaviors, for example, the nodes users are interested in. **Note that Lightning explorers just illustrate information that can already be queried from the gossip protocol (that's how they obtained those data) and those data can be easily acquired via the command line in LAPPs.** So it is recommended to use commanline tools or a privacy-preserving browser (e.g., Tor browser), if a user care about her privacy.
