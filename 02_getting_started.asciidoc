...
However, using Lightning explorer may introduce privacy issue, because, if users are careless, the website may track their IP addresses and collect their behaviors, for example, the nodes users are interested in. 

[WARNING]
====
Note that Lightning explorers just illustrate information that can already be queried from the gossip protocol (that's also how they obtain those data) and those data can be easily acquired via the command line in LAPPs. So it is recommended to use commanline tools or a privacy-preserving browser (e.g., Tor browser), if a user care about her privacy.
====
