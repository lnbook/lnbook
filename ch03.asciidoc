...
Payment channels are nothing else than a 2 out of 2 multisignature address on the Bitcoin Network for which you hold a key and your channel partner holds the other key.
...
The design will discourage you and your channel partner from publishing an old (outdated) state of your channel.
...
As it is a rather smart construction, one might call it a smart contract.
...
Last, but not least, it is important to understand that the LN is nothing else but Bitcoin.
...
Besides all the technical primitives, the LN protocol is a creative way to get the most use out of Bitcoin by allowing an arbitrary amount of instant payments with instant settlements without the necessity to trust anyone else but the Bitcoin network.
...
The latter seem much easier to implement and have a couple of desirable properties.
However, they require a new OP_CODE be added to the Bitcoin scripting language and can't currently be implemented on bitcoin mainnet.

==== Multisig addresses
Payment channels are smart contracts that build on top of 2-of-2 multisignature addresses which will be spent by Segwit transactions.
...
Thus, we will not review all the technical details about multisignature addresses and Segwit but just stick to the properties.
...
[Note]
====
You will often hear people complain about bitcoin being locked in the Lightning Network which can't move freely.
