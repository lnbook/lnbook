...
In practical terms, instant settlement means that payments take on the order of 1 second to execute.
...
In a direct payment where payer and payee are connected via a channel, the limit is states as above. 
...
There must be enough capacity on every single channel that the payment is routed through.
