...
As will be explained, each node opens the box (using a cryptographic key), takes out the onion, and peals off the outermost layer of this onion.
The receiving node can only read this outermost layer, no further layers, achieved through cryptography.
This outermost layer tells the node if the box needs to be routed further and to which node. 
