...
In order to activate a plugin in +c-lightning+, you need to first make sure the plugin script is executable. For example, after downloading the autopilot plugin and placing the script into +~/.lightningd/+, you can make it executable with the +chmod+ command:

----
   chmod +x ~/.lightningd/plugins/autopilot.py
----

Then, in order to activate the script, stop your +lightningd+ and restart it with the +plugin+ argument like this:

----
   lightningd --plugin=~/.lightningd/plugins/autopilot.py
----
