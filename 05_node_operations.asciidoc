...
In order to activate a plugin in +c-lightning+, place it into the +~/.lightning/plugins+ directory and ensure that it's executable, e.g. +chmod +x ~/.lightning/plugins/autopilot.py+, then restart +c-lightning+.
