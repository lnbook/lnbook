...
== Private channels
If you intend to use the Lightning Network to send and receive funds between nodes and wallets you control, there is little need to open public channels to the rest of the network.
Instead you can open a private channel between, say, your desktop PC running a full node and a Lightning Wallet on your mobile phone.
Since you are not exposing your channel to the network, you eliminate the risk of a denial-of-service attack on your node.
You can more easily manage the capacity of this channel, since it will only be used to receive or send directly to one's node.

You could also open a private channel with a known party that you transact with frequently
For example, if Alice and Bob frequently send money back and forth to each other, they could open a private channel.
This channel will not be able to be used to route payments from other users and collect fees.
The advantage is that the existence of this channel will not be known to the rest of the network, and any payments between Alice and Bob cannot be inferred by tracking changes in the channel's routing capacity.
However, final balance of the channel will be made public once the channel is closed and the balance is committed to the Bitcoin blockchain.
But as this commitment transaction is pseudonymous, it will not be a simple matter to connect it back to Alice or Bob.
