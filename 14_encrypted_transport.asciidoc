...
handshake has been completed, both nodes can now begin to send each other
...
2. Parse the read message (`m`) into `v`, `re`, and `c`:
    ...
    * The raw bytes of the remote party's ephemeral public key (`re`) are to be
      ...
      by the key's serialized compressed format.
...
During Act Three, the initiator transmits its static public key to the
