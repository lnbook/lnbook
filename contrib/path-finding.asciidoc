...
** Why is liquidity a problem? Why do channel balances have to be private and what is the problem with that? 
...
## Generating candidate paths by solving / estimating the optimizaion problem
...
The sender now uses the same path to send the payment with the current payment hash.
...
For simplicity we take the liberty to also abbreviate multipath payments with MPP.
====
...
Those multipart payments could again be split down if they are not successful along a route.
...
While proactive rebalancing increases the reliability of the network it is currently economically not viable.
...
However no one stops your node from shortly interrupting the routing process and conduct a rebalancing operation to provide yourself with sufficient liquidity on the channel in question.
...
There is another way how multipart payments and rebalancing can be combined.
