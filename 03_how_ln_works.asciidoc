...
The invoices contain the amount of bitcoin that is requested and a signature of the payee.
...
As a simplified physical analogy imagine the payment package as a sealed box with an onion inside. 
...
On the Lightning Network, confirmations only matter for opening and closing channels on-chain.
...
The transaction is registered in the blockchain, and at any later point in time the recipient can have a look at it and see that it has received funds in the past. 
