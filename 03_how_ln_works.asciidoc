...
. Onions can have up to around 26 hops or onion layers if you prefer. This allows for sufficiently long paths. The precise path length avaiblael dpeneds on the amount of bytes allocated to the routing payload at each hop.
