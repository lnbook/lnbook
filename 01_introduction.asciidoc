...
Before we get into how the Lightning Network works, it's important to understand one basic concept that underlies Bitcoin, the Lightning Network and many other such systems: something we call a _fairness protocol_. A fairness proctocol is a way to _achieve fair outcomes between participants, who do not need to trust each other and without the need for a central authority_. It is the backbone of decentralized systems like Bitcoin.
...
* Require trust - you only interact with people who you already trust due to prior interactions, reputation, or familial relationships. This works well enough within families and other small groups where it is the most common basis for cooperative behavior. Unfortunately, it doesn't scale and it suffers from tribalist (in-group) bias.
...
[WARNING]
====
While the infamous chip battles of the 1980s neatly illustrate the point, any similarity between the scenario above and any of the authors' actual childhood experiences with their cousins is entirely coincidental... Or is it?
====
...
This is only a small list of a whole "menagerie" of security and cryptographic primitives that are in use. Additional basic primitives and combinations are invented all the time.
...
Again and again, we see this pattern. Fair outcomes are not enforced by any authority. They emerge as the natural consequence of a protocol that rewards fairness and punishes cheating. This fairness protocol harnesses self-interest by directing it towards fair outcomes.
...
While 40,000 transactions per second seems like a lot, it only achieves parity with traditional financial payment networks at peak times. Innovations in machine-to-machine payments, micro-transactions and other applications are likely to push demand to many orders higher than this.
...
In February 2015, Joseph Poon and Thaddeus Dryja proposed a possible solution to the Bitcoin Scalability Problem, with the publication of _The Bitcoin Lightning Network: Scalable Off-Chain Instant Payments_
...
Aside from reducing the burden on nodes, payments on the Lightning Network are cheaper for users as they do not need to pay blockchain fees, and more private for users as they are not published on a public ledger and furthermore, are not stored permanently.
