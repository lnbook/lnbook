...
The Lightning Network is a second-layer protocol on top of Bitcoin that enables
instant, low-cost Bitcoin payments. Unlike the base blockchain, the Lightning
netowrk is able able to properly send micropayments, down to _fractions_ of a
satoshi.
...
Scalable Off-Chain Instant Payments" (https://lightning.network/lightning-network-paper.pdf).] In the original whitepaper, Poon and Dryja estimate that in order for Bitcoin to reach the 47,000 transactions per second processed at peak by Visa, it would require 8 GB blocks. This would make running a node completely untenable for anyone but large scale enterprises and industrial grade operations. The result would be a network in which only a few users can actually validate the state of the ledger, which ultimately breaks the trust model of Bitcoin.

The Lightning Network proposes a new network, a "second layer", where users can make transaction with each other in a peer-to-peer manner, without the necessity to publish a transaction to the Bitcoin blockchain every time.
Users may transaction on the Lightning Network as many times with as many users as they want, making use of the Bitcoin blockchain only in order to load bitcoin onto the network initially and to "settle", that is: remove bitcoin from the Lightning Network.
...
Aside from reducing the burden on nodes, these transactions on the Lightning Network will be cheaper for users as they do not need to pay blockchain fees, and more private for users as they are not published to all participants of the network.
 ...
 * Users of the Lightning Network can route payments to each other for very low cost and in real time. These "routers" can also earn a fee for their service (payment routing).
