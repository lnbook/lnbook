...
This allows LN to rely on the HTLC mechanism (which uses the preimage of a hash function) for the atomicity of multi-hop payments: nobody except the final recipient can reveal the payment secret and resolve the HTLC.
...
Finally, as a payment network, LN depends on economic stimuli.
...
In summary, while some aspects of the Lightning Network's architecture suggest that it is a step forward in terms of privacy compared to Bitcoin, other properties of the protocol may make attacks on privacy easier. Thorough research is needed to evaluate what privacy guarantees LN provides and improve the state of affairs.
...
Recent research describes various ways in which the security and privacy of LN may be compromised.
...
While the public announcement of IP addresses may be unavoidable for those nodes that wish to have incoming channels in LN, linkability across nodes of the same user can be mitigated if the clients for each node are hosted with different service providers and thus IP addresses.
...
These cross-layer linking algorithms could be foiled if users possess multiple unclustered addresses or use multiple wallets to interact with LN.
...
In this section, we discuss and analyze LN from the point of view of network science.
...
In LN, nodes represent LN nodes, and edges represent payment channels between them.
...
If this was the case, then the degree distribution would follow a Gaussian normal distribution.
...
Whenever a node joins the network, it may want to maximize its connectivity to other nodes in order to increase its routing efficiency. This phenomenon is called preferential attachment.
These economic incentives result in a fundamentally different network than a random graph.

Based on snapshots of publicly announced channels, the degree distribution of the network follows a power-law function.
...
It turns out that if the adversary chooses nodes randomly, then scale-free networks like LN are robust against node-removal attacks.
...
Not surprisingly, LN and other scale-free networks are _not_ robust against targeted node-removal attacks.
...
LN is a dynamically changing, permissionless network.
...
temporal_ln]]
.The steady growth of LN in terms of nodes, channels and locked capacity.
...
However, this behavior would be strange in LN.
...
Surprisingly, triangle closing is a common practice in LN.
...
We can observe that LN has a greater central point dominance, ie. it is more centralized, than a random graph (Erdős-Rényi graph) or a scale-free graph (Barabási-Albert graph) of equal size.
However, we also note, that over time the central point dominance of LN gradually decreases as the network grows and densifies, as shown in <<central_point_dominance_ln>>.

[[central_point_dominance_ln]]
.Central point dominance of LN, a random graph (ER) and a scale-free graph (BA) of equal size.
...
It is fruitful to analyze how protocol changes like multi-part payments can affect the dynamics of LN.
