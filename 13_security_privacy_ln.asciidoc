...
In this chapter, we look at some of the most important issues related to the security and privacy of the Lightning Network. First, we'll consider privacy, what it means, how to evaluate it, and some things you can do to protect your own privacy while using the LN. Then we'll explore some common attacks and mitigation techniques.
...
=== Differences between the LN and Bitcoin in terms of privacy
...
Consequently, for a complete analysis of the LN's privacy provisions, one needs to consider every layer of the technological stack users might interact with.
...
In the case of the LN, these attributes can represent a channel's capacity.
...
This might change in the future if the LN has significantly larger traffic or if a market for routing fees emerge.
