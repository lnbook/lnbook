...
The `node_announcement` message allows a node to announce/update its vertex within the
...
The `announce_signatures` message is exchanged by channel peers to
...
signatures required to generate an `announce_signatures` message.

==== Channel Graph Syncing

The `query_short_chan_ids` message allows a peer to obtain the channel information
...
The `reply_channel_range` message is the response to the `query_channel_range` message and
