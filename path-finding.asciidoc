...
[[rebalancing-3]]
.caption
image:images/rebalancing-3.png[]
...
We can see that in this graph that Bob, Wei and Erica all have channels with a capacity of 5.000.000 satoshi with each other.
Everyone of them owns 2.500.000 Satoshi in their channel.
Let us assume Bob wanted to send Wei 1 million satoshi.
We would have 

rebalancing will leave others with uneven state.

IMPORT PICTURE

even the most rebalanced state that cannot reach 50/50 everywhere for obious reasons

IMPORT PICTURE

a better goal is low imbalance measured via gini on relative funds. Explain why releative and how this is actually done

Show it still can lead into conflicting situations.
