...
Ephemeral Key::
    ...
    Even if an ephemeral key leak, only information about a single payment becomes public.
...
Gossip Protocol::
    ...
    Usually Lightning nodes connect with their channel partners, but it is fine connected with any other Lightning node in order to process gossip messages.
