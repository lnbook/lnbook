...
Transaction Malleability::
    Transaction Malleability is a property that a transaction's hash can change without real changes in transaction content. For example, an unsigned transaction's hash will change after signing, which is inconvenient and may lead to issues. 
    Segregated Witness sorf fork addresses this issue and therefore is an important upgrade to support Lightning network: if the transaction hash keep changing then it's hard to track the payment status.
