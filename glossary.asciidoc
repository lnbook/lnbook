...
Autopilot::
    The autopilot is a recommendation engine for LN nodes that uses statistics of the known topology to suggest which nodes they should open channels with.
    Depending on the implementation of the autopilot, the channel capacity may also be recommended.
    Autopilots are not part of the LN Protocol.
...
BOLT::
    ...
    The formal specification of the LN Protocol is called Basics Of Lightning Technology.
...
c-lightning::
    Implementation of the LN Protocol by the Victoria based Blockstream. It is written in C.
...
Diffie Hellman Key Exchange::
    On the LN, the Elliptic Curve Diffie-Hellman method is used.
...
Eclair::
    Implementation of the LN Protocol by the Paris based company ACINQ. It is written in Scala.

encoding::
    tbd.

Ephemeral Key::
    Ephemeral keys are mainly within the SPHINX Mix Format and Onion Routing on the LN.
...
fees::
    ...
    Not to be confused with a routing fee for payments on the LN.
    Nodes on the LN are allowed to take a routing fee for forwarding payments.
...
Globalfeatures::
    Globalfeaturess of a LN node are the features of interest for all other nodes.
...
Gossip Protocol::
    LN nodes send and receive information about the topology of the LN through gossip messages which are exchanged with their peers.
...
hardware wallet::
    ...
    Currently hardware wallets are not available for LN nodes as user nodes need to be online to follow through the protocol.
...
HTLC::
    ...
    On the LN HTLCs are outputs in the commitment transaction of a payment channel and are used to enable the trustless routing of payments.

Invoice::
    The Payment process on the LN is initiated by the payee who issues an invoice.
...
Lightning Network (LN)::
   The Lightning Network (LN) is a protocol on top of bitcoin (or other cryptocurrencies).
   ...
   Other components of the LN are the gossip protocol, the transport layer and payment requests.

LN Node::
    tbd.

lnd::
    Implementation of the LN Protocol by the San Francisco based company Lightning Labs.
    It is written in Go.

Localfeatures::
    Localfeaturess of a LN node are the features of direct interest of the peer.
    They are announced in the `_init_` message of the peer protocol as well as the `_channel_announcement_` and `_node_announcement_` messages of the gossip protocol.
...
Millisatoshi::
    The smallest unit of account on the LN.
...
Noise_XK::
    The template of the Noise protocol framework to establish and authenticated and encrypted communication channel between two peers of the LN.
...
Payment::
    A payment occurs if we transfer bitcoin within the LN.
...
Payment Channel::
    Payment Channels are the core building blocks of the LN.
    ...
    In the LN whitepaper, Joseph Poon and Tadge Dryja describe the Revocable Sequence Maturity Contract based method that is currently being implemented on the LN.
...
Topology::
    The topology of the LN describes the shape of the LN as a mathematical graph.
    Nodes of the graph are the LN nodes or participants.
    The edges of the graph are the payment channels.
    The topology of the LN is publicly broadcasted with the help of the gossip protocol unless nodes decide to act privately.
    This means that the LN is probably larger than the announced number of nodes.
...
Source-Based Routing::
    On the LN the sender of a payment decides the route of the payment.
...
SPHINX Mix Format::
    A particular technique for Onion Routing used in the LN and invented by George Danezis and Ian Goldberg in 2009.
