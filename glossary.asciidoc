...
funding transaction::
    ...
    It is spent by the commitment transactions or by the closing transaction.
...
Noise_XK::
    ...
    More particularly (from: http://www.noiseprotocol.org/noise.html), the protocol enables encryption to a known recipient and strong forward secrecy. This payload is encrypted based on an ephemeral-ephemeral DH as well as an ephemeral-static DH with the recipient's static key pair. Assuming the ephemeral private keys are secure, and the recipient is not being actively impersonated by an attacker that has stolen its static private key, this payload cannot be decrypted. Sender authentication is resistant to key-compromise impersonation (KCI). The sender authentication is based on an ephemeral-static DH ("es" or "se") between the sender's static key pair and the recipient's ephemeral key pair. Assuming the corresponding private keys are secure, this authentication cannot be forged.
...
payment::
    A payment occurs if bitcoin is stransferred within the Lightning Network.
...
private channel:
    ...
    As the number and capacity of unanounced channels is unknown, the total public channel count and capacity is only a portion of the total Lightning Network.
...
Revocable Sequence Maturity Contract::
    ...
    Unlike an HTLC, whose timeout is to make a HTLC temporary and which therefore should be absolute, a RSMC timeout is meant to only start when a commitment transaction is mined, and therefore should be using a Relative Timelock.
...
transport layer::
    In computer networking, the transport layer is a conceptual model of how computers talk to each other.
...
wallet::
    A walledt is a piece of software that holds Bitcoin addresses and secret keys. It is used to send, receive, and store bitcoin.
    
watchtower::
    Watchtowers are a security service on the Lightning network that monitor channels.
    In case one of the channel partners goes offline or loses their backup, a watchtower keeps its own backups and can restore channel information.
    They also monitor the Bitcoin blockchain and can submit a penalty transaction in the event that one of the partners tries to "cheat" by broadcasting an outdated state.
